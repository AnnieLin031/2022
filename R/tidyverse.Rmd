...
path <- system.file("extdata", package="dslabs")   ##can't find the path
filename <- file.path(path, "fertility-two-countries-example.csv")   #why not system.file??
...
```{r, echo=FALSE}
co2
class(co2)  ##ts: time-series object
is.data.frame(co2)
is.matrix(co2)
is.vector(co2)
class(iris)
...
B (what's the definition of tidy data???)
```{r, echo=FALSE}
ChickWeight
class(ChickWeight)
```
...
```{r}
new_table <- select(murders, state, region, rate)  ##select(data,colname A, colname B,...)
...
my_states <- mutate(murders, rate=total/population*100000) %>%
  ...
  filter(region== c("Northeast", "West") & rate <1) %>%   ##why can't use "=="??
  select(state, rate, rank, region)
```
...
```{r}
heights |> group_by(sex)  ##group_by:a tidyverse function(開頭多了tibble和groups)
```
...
heights |> 
  group_by(sex) |>
  summarize(average = mean(height), standard_deviation = sd(height)) 
##如果sex是factor=>會依level排，如果是character=>會依字母順序派
```
...
murders |> 
  ...
  summarize(median_min_max(rate),mean(rank))
```
...
us_murder_rate <- murders |> 
  summarize(rate = sum(total) / sum(population) * 100000) |>
  pull(rate)    #原本是data.frame, pull後會變成numeric (vector)  ###9/12 stops here###
```
...
murders |>
  rank(rate) |>
  head()    ##arrange: from small to large   ##why can't use rank??
...
```{r, eval=FALSE}
murders |> 
  arrange(desc(rate)) ##desc: just like -
```
...
```{r}
murders |> 
  arrange(region,rate) |>   #先依region、同一region再由rate排
  head()
```
